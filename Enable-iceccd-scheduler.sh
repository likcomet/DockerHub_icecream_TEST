...
case $ICECREAM_SCHEDULER in
	yes) icecc-scheduler -d -n neople -l /var/log/icecc-scheduler.log -vvvv
             iceccd -d -s $ICECREAM_SCHEDULER_HOST -m 3 -n neople -l /var/log/iceccd.log;tail -f /var/log/icecc*.log
	;;
	no)  iceccd -d -s $ICECREAM_SCHEDULER_HOST -m 3 -n neople -l /var/log/iceccd.log;tail -f /var/log/icecc*.log
	;;
	*)   echo "Null"
	;;
esac
